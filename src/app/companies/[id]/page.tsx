...
export default function CompanyProfilePage() {
  ...
  const [sessionRole, setSessionRole] = useState<"guest" | "buyer" | "vendor" | "admin">("guest");
  const [buyer, setBuyer] = useState<BuyerOrganization | null>(null);
  ...
  useEffect(() => {
    void (async () => {
      ...
      setSessionRole(payload.role ?? "guest");
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const payload = (await response.json().catch(() => ({}))) as { companyIds?: string[] };
      setFavoriteCompanyIds(Array.isArray(payload.companyIds) ? payload.companyIds : []);
      setFavoritesLoading(false);
    })();
  }, [sessionRole]);
              ...
              {company.completedEngagements && company.completedEngagements.length > 0 ? (
                <div className="grid gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">{locale === "ja" ? "OffshoreMatch上で完了した案件履歴です。" : "Engagements completed through OffshoreMatch."}</p>
                  </div>
                  <div className="grid gap-3">
